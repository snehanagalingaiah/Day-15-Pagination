...
//Fetching all html elements
  ...
  let dynBtn = document.getElementById("dyn-btn");
  var pre_next_person;


// onload of the window display the first reord
  window.addEventListener("DOMContentLoaded",()=>{
    var currentRecord = 1;
    showPerson(currentRecord);
  });
 
//function to create dynamic buttons between prev and next button
  function pageButtons(currentRecord){

  const total_pages_per_window = 5; //total 5 buttons per page other than prev and next
  var maxLeft = currentRecord - Math.floor(total_pages_per_window/2); // to calculate the left most button. eg if 5th record is selected 3 will be the left most button
  var maxRight =  currentRecord + Math.floor(total_pages_per_window/2);// to calculate the right most button. eg if 5th record is selected 7 will be the right most button.
   
  dynBtn.innerHTML = " ";

  if (maxLeft < 1) {
    maxLeft = 1
    maxRight = total_pages_per_window;
}


if (maxRight > records.length) {
  maxLeft = currentRecord - (total_pages_per_window- 1)
  
  if (maxLeft < 1){
    maxLeft = 1
  }
  maxRight = records.length;
}

  for (currentRecord = maxLeft; currentRecord <= maxRight; currentRecord++) 
  {
    dynBtn.innerHTML += `<button class="btn btn-outline-primary" value=${currentRecord} onclick="showPerson(${currentRecord})">${currentRecord}</button>`;

  }

 if(pre_next_person!=1){
  dynBtn.innerHTML = `<button class="btn btn-outline-primary"  value=${1} onclick="showPerson(${1})">First</button>` +` <button class="btn btn-outline-primary" value=${pre_next_person-1} onclick="showPerson(${pre_next_person-1})">Prev</button>` + dynBtn.innerHTML;
 }

 if(pre_next_person!=records.length){
  dynBtn.innerHTML += ` <button class="btn btn-outline-primary"  value=${pre_next_person+1} onclick="showPerson(${pre_next_person+1})">Next</button>` + ` <button class="btn btn-outline-primary" value=${records.length} onclick="showPerson(${records.length})">Last</button>`
 }

 console.log("pre_nxt_person", pre_next_person );
